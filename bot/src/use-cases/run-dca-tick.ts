...
export class RunDcaTickUseCase {
	...
	async tick(): Promise<void> {
		const { registry, chain, gateway, quotes, executor, guard, strategy, logger } =
			this.deps
		...
		for (const { smartAccount, user } of accounts) {
			try {
				...
				const health = await chain.readStreamHealth(
					strategy.superTokenIn,
					user,
				)
				if (shouldCloseStream(health, closeThresholdBps)) {
					const txHash = await guard.closeStream(
						smartAccount,
						strategy.superTokenIn,
					)
					logger.info(
				...
				try {
					superBalance = await chain.readErc20Balance(
						strategy.superTokenIn,
						smartAccount,
					)
				} catch {
				...
				if (!decision) {
					logger.info(
						{ smartAccount, owner: user },
						'no_trade_insufficient_balance_or_no_targets',
					)
					continue
				}
				...
				logger.error(
					{ smartAccount, err: formatError(err) },
					'account_iteration_failed',
				)
			}
		}
	}
