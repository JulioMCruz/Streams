 ...
 *   The function currently panics; test is marked EXPECTED TO FAIL.
 */

import { expect } from 'chai'

import type { SmartAccountState } from '../../src/domain/models/smart-account.js'
import { decideSwap, type StrategyTokens } from '../../src/domain/strategy.js'
...
describe('strategy', function () {
  describe('decideSwap', function () {
    // ── Happy paths ────────────────────────────────────────────────────

    it('Should return a SwapDecision when balance exceeds minTradeAmount', function () {
      ...
      expect(decision!.smartAccount).to.equal(SA)
      expect(decision!.superTokenIn).to.equal(USDCX)
      // 90% of the balance is traded (10% headroom for the read→execute race).
      expect(decision!.superAmountIn).to.equal(4_500_000_000_000_000_000n)
      expect(decision!.tokenIn).to.equal(USDC)
      expect(decision!.tokenOut).to.equal(WETH)
      expect(decision!.underlyingAmountIn).to.equal(4_500_000n) // 90% of 5 USDC
      expect(decision!.maxSlippageBps).to.equal(100)
    })

    it('Should pick the first target token from the array', function () {
      const state = makeState({ targetTokens: [WBTC, WETH] })
      const decision = decideSwap(state, 2_000_000_000_000_000_000n, defaultTokens)

      expect(decision).to.not.be.null
      expect(decision!.tokenOut).to.equal(WBTC) // first one wins
    })

    it('Should correctly compute underlyingAmountIn using integer division', function () {
      ...
      expect(decision!.underlyingAmountIn).to.equal(1_350_000n)
    })

    it('Should pass maxSlippageBps from state unchanged to the decision', function () {
      const state = makeState({ maxSlippageBps: 300 })
      const decision = decideSwap(state, 5_000_000_000_000_000_000n, defaultTokens)
      expect(decision!.maxSlippageBps).to.equal(300)
    })

    // ── Null returns (skip conditions) ────────────────────────────────

    it('Should return null when targetTokens is empty', function () {
      const state = makeState({ targetTokens: [] })
      const decision = decideSwap(state, 5_000_000_000_000_000_000n, defaultTokens)
      expect(decision).to.be.null
    })

    it('Should return null when underlyingAmountIn is exactly equal to minTradeAmount - 1', function () {
    ...
    it('Should proceed when the margined underlyingAmountIn equals minTradeAmount exactly', function () {
      // 1 USDCx * 0.9 = 0.9 USDC margined; with minTradeAmount = 900_000 the
      // guard `< minTradeAmount` is false at equality, so it proceeds.
      const superBalance = 1_000_000_000_000_000_000n // exactly 1 USDCx
      const state = makeState({ minTradeAmount: 900_000n })
      const decision = decideSwap(state, superBalance, defaultTokens)
      // underlyingAmountIn = 900_000n which is NOT < 900_000n → proceed
      expect(decision).to.not.be.null
    })

    it('Should return null when superBalance is 0', function () {
      const state = makeState()
      const decision = decideSwap(state, 0n, defaultTokens)
      expect(decision).to.be.null
    })

    it('Should return null when superBalance produces underlyingAmountIn === 0 due to truncation', function () {
    ...
    it('Should handle minTradeAmount === 0 (always trade if balance > 0)', function () {
      const state = makeState({ minTradeAmount: 0n })
      // Even a tiny balance passes when minTradeAmount is 0
    ...
    it('Should handle a single-token targetTokens array correctly', function () {
      const state = makeState({ targetTokens: [WBTC] })
      const decision = decideSwap(state, 5_000_000_000_000_000_000n, defaultTokens)
      expect(decision!.tokenOut).to.equal(WBTC)
    })

    it('Should not mutate the state.targetTokens array', function () {
      const tokens = [WETH, WBTC]
      const state = makeState({ targetTokens: tokens })
      decideSwap(state, 5_000_000_000_000_000_000n, defaultTokens)
      expect(state.targetTokens).to.deep.equal([WETH, WBTC])
    })

    it('Should work with a divisor of 1 (same-decimals token pair)', function () {
      ...
      expect(decision!.underlyingAmountIn).to.equal(4_500n)
    })

    it('Should propagate superAmountIn as 90% of superBalance (read→execute headroom)', function () {
      const superBalance = 7_500_000_000_000_000_000n
      const state = makeState()
      const decision = decideSwap(state, superBalance, defaultTokens)
      // 7.5e18 * 9000 / 10000 = 6.75e18
      expect(decision!.superAmountIn).to.equal(6_750_000_000_000_000_000n)
    })

    // ── Adversarial tests ─────────────────────────────────────────────

    it('Should return null when targetTokens has a falsy first element', function () {
      // Although Address is always a string in TypeScript, an empty string or
      // zero-address could slip through at runtime. The function checks
      // `if (!tokenOut) return null` — verify that guard fires for an empty string.
    ...
    it('Should handle very large superBalance without throwing', function () {
      const MAX_UINT256 = 2n ** 256n - 1n
      const state = makeState({ minTradeAmount: 1n })
      // Should not throw — just compute a large underlyingAmountIn
      expect(() => decideSwap(state, MAX_UINT256, defaultTokens)).to.not.throw()
    })
  })
})
